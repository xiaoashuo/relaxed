...
public class SftpTest {
	 ...
	 * 依赖包 部分方法 会切换当前工作目录,导致获取文件不到 后续版本 调整完成后回到根目录(已修复) 替代解决方案 所有的的路径使用绝对路径 测试复现方法如下
	 * sftp.getChannelSftp().cd("/");
	 * sftp.delete("tianze3/55030002/upload","BILLING_20241221.ok"); String pwd =
	 * sftp.getChannelSftp().pwd(); boolean exista =
