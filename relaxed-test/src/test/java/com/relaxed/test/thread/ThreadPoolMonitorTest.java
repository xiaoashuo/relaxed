...
public class ThreadPoolMonitorTest {
	...
	public static void monitorTest() {
		AlertService alertService = new AlertService() {
			@Override
			public void sendAlert(String finalMsg, String[] channels) {
		...
		ThreadPoolTaskMonitor threadPoolTaskMonitor = new ThreadPoolTaskMonitor(alertService, monitorProperties);

		orderProcessExecutor = threadPoolTaskMonitor.register("订单线程池", orderProcessExecutor);
		...
		for (int i = 0; i < 1000; i++) {
			PoolOrder poolOrder = new PoolOrder();
			poolOrder.setUsername("username" + i);
			jobs.add(processOrderAsync(poolOrder, orderProcessExecutor));
		}
