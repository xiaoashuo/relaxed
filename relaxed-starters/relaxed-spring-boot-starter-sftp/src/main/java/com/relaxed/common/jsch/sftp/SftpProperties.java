 ...
 */
@Data
@ConfigurationProperties(prefix = "relaxed.sftp")
public class SftpProperties {
	 ...
	 * 会话连接超时时间
	 */
	private Integer sessionConnectTimeout = 0;

	/**
	 * 渠道连接超时时间
	 */
	private Integer channelConnectedTimeout = 0;
	...
	public static class Pool {
		 ...
		 * 可以根据需要自己调整，单位是毫秒。 默认值是5000L
		 */
		private long maxWaitMillis = DEFAULT_MAX_WAIT_MILLIS;

		/**
		 * 对象最小的空闲时间。如果为小于等于0，最Long的最大值，如果大于0，当空闲的时间大于这个值时，执行移除这个对象操作。 可以避免连接泄漏 默认值是-1
		 */
		 ...
		 * (evictor继承自TimerTask，由ScheduledThreadPoolExecutor进行调度) 默认值是10000L，即10秒
		 */
		 ...
		 * 这个和上面的minEvictableIdleTimeMillis的区别是，它会保留最小的空闲对象数量。而上面的不会，是强制性移除的。 默认值是1000L *
		 * 60L * 30L，即30分钟
		 */
		private long softMinEvictableIdleTimeMillis = DEFAULT_SOFT_MIN_EVICTABLE_IDLE_TIME_MILLIS;

		/**
		 * 检测空闲对象线程每次检测的空闲对象的数量。如果这个值小于0，则每次检测的空闲对象数量等于当前空闲对象数量除以这个值的绝对值，并对结果向上取整。 默认值是3
		 */
		private int numTestsPerEvictionRun = BaseObjectPoolConfig.DEFAULT_NUM_TESTS_PER_EVICTION_RUN;

		/**
		 * 在创建对象时检测对象是否有效，true是，默认值是false。做了这个配置会降低性能。 默认值是false
		 */
		private boolean testOnCreate = BaseObjectPoolConfig.DEFAULT_TEST_ON_CREATE;

		/**
		 * 在从对象池获取对象时是否检测对象有效，true是；默认值是false。做了这个配置会降低性能。 默认值是false
		 */
		private boolean testOnBorrow = BaseObjectPoolConfig.DEFAULT_TEST_ON_BORROW;

		/**
		 * 在向对象池中归还对象时是否检测对象有效，true是，默认值是false。做了这个配置会降低性能。 默认值是false
		 */
		...
		/**
		 * 空闲对象检测线程的执行周期，即多长时间执行一次空闲对象检测。 单位是毫秒数。如果小于等于0，则不执行检测线程。 默认值是1000L * 60L *
		 * 10L，即10分钟
		 */
		...
		/**
		 * 使用jmxNameBase + jmxNamePrefix + i来生成ObjectName 默认值是sftp
		 */
	...
	public static class Abandoned {
		...
		/**
		 * 从对象池中获取对象的时候进行清理 如果当前对象池中少于2个空闲状态的对象或者 活跃数量>最大对象数-3 的时候，在获取对象的时候启动泄漏清理 默认值是true
		 */
		private boolean removeAbandonedOnBorrow = DEFAULT_REMOVE_ABANDONED_ON_BORROW;

		/**
		 * 池维护（evicor）是否执行放弃的对象删除。
		 * 必须将连接池的timeBetweenEvictionRunsMillis属性设置为一个有效的值，否者此配置不生效 默认值是true
		 */
		private boolean removeAbandonedOnMaintenance = DEFAULT_REMOVE_ABANDONED_ON_MAINTENANCE;

		/**
		 * 对象被获取后多长时间没有返回给对象池，则放弃对象，单位秒 默认值是300
		 */
		private int removeAbandonedTimeout = DEFAULT_REMOVE_ABANDONED_TIMEOUT;

		/**
		 * 是否记录放弃对象的应用程序代码的堆栈跟踪， 废弃对象的日志记录会为每个创建的对象增加开销，因为必须生成堆栈跟踪。 默认值是false
		 */
		private boolean logAbandoned = DEFAULT_LOG_ABANDONED;

		/**
		 * 当logAbandoned为true时有效， 是否记录完整堆栈信息，如果禁用此功能，则可以使用更快但信息量较少的堆栈遍历机制（如果可用）。 默认值是false
		 */
		private boolean requireFullStackTrace = DEFAULT_REQUIRE_FULL_STACK_TRACE;

		/**
		 * 如果池实现了org.apache.commons.pool2.UsageTracking接口，是否记录完整堆栈信息用来辅助调试废弃对象 默认值是false
		 */
