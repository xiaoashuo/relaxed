...
public class HttpInputStream extends InputStream {
	 ...
	 * @param response 响应对象
	 */
	public HttpInputStream(com.relaxed.common.core.util.http.HttpResponse response) {
		init(response);
	}
	 ...
	 * @param response 响应对象
	 */
	private void init(com.relaxed.common.core.util.http.HttpResponse response) {
		try {
			this.in = (response.status < HttpStatus.HTTP_BAD_REQUEST) ? response.httpConnection.getInputStream()
		...
		if (null == this.in) {
			this.in = new ByteArrayInputStream(
					StrUtil.format("Error request, response status: {}", response.status).getBytes());
			return;
		}
