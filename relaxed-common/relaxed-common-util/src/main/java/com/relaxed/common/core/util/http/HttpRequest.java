...
import com.relaxed.common.core.util.http.body.BytesBody;
import com.relaxed.common.core.util.http.body.FormUrlEncodedBody;
import com.relaxed.common.core.util.http.body.MultipartBody;
import com.relaxed.common.core.util.http.body.RequestBody;
import com.relaxed.common.core.util.http.cookie.GlobalCookieManager;
import com.relaxed.common.core.util.http.interceptor.HttpInterceptor;
import com.relaxed.common.core.util.http.part.BytePart;
import com.relaxed.common.core.util.http.part.FilePart;
import com.relaxed.common.core.util.http.part.MultiFilePart;
 ...
 */
public class HttpRequest extends com.relaxed.common.core.util.http.HttpBase<HttpRequest> {
	...
	public static HttpRequest of(String url) {
		return of(url, com.relaxed.common.core.util.http.HttpGlobalConfig.isDecodeUrl() ? DEFAULT_CHARSET : null);
	}
	 ...
	 * @see com.relaxed.common.core.util.http.HttpGlobalConfig#setTimeout(int)
	 * @since 4.6.2
	 */
	public static void setGlobalTimeout(int customTimeout) {
		com.relaxed.common.core.util.http.HttpGlobalConfig.setTimeout(customTimeout);
	}
	...
	private com.relaxed.common.core.util.http.HttpConfig config = com.relaxed.common.core.util.http.HttpConfig.create();
	...
	public HttpRequest(UrlBuilder url) {
		...
		// 给定一个默认头信息
		this.header(com.relaxed.common.core.util.http.GlobalHeaders.INSTANCE.headers);
	...
	public HttpRequest form(String name, Object value) {
		...
		if (value instanceof Part) {
			// part 部分
			return this.form(name, (Part) value);
		}
		// 普通值
		String strValue;
	...
	public HttpRequest form(String name, File... files) {
		...
		return form(name, new MultiFilePart(files));
	}
	...
	public HttpRequest form(String name, File file, String fileName) {
		if (null != file) {
			form(name, new FilePart(fileName, file));
		}
		return this;
	}
	...
	public HttpRequest form(String name, byte[] fileBytes, String fileName) {
		if (null != fileBytes) {
			form(name, new BytePart(fileBytes, fileName));
		}
		return this;
	}
	 ...
	 * @since 4.0.9
	 */
	public HttpRequest form(String name, Part resource) {
		if (null != resource) {
	 ...
	 * @since 3.3.0
	 */
	public Map<String, Part> fileForm() {
		final Map<String, Part> result = MapUtil.newHashMap();
		this.form.forEach((key, value) -> {
			if (value instanceof Part) {
				result.put(key, (Part) value);
			}
		});
		return result;
	}
	...
	public HttpRequest body(String body, String contentType) {
		...
		if (null != contentType) {
			...
			// 在用户未自定义的情况下自动根据内容判断
			contentType = com.relaxed.common.core.util.http.HttpUtil.getContentTypeByRequestBody(body);
			if (null != contentType && ContentType.isDefault(this.header(Header.CONTENT_TYPE))) {
				if (null != this.charset) {
					// 附加编码信息
					contentType = cn.hutool.http.ContentType.build(contentType, this.charset);
				}
				this.contentType(contentType);
			}
		}
	 ...
	 * @return this
	 */
	public HttpRequest setConfig(com.relaxed.common.core.util.http.HttpConfig config) {
		this.config = config;
		return this;
	}
	 ...
	 * @param protocol 协议
	 * @return this
	 * @see cn.hutool.core.net.SSLUtil#createSSLContext(String)
	 ...
	 * @return this
	 */
	public com.relaxed.common.core.util.http.HttpResponse execute() {
		return this.execute(false);
	}

	/**
	 * 异步请求<br>
	 * 异步请求后获取的{@link cn.hutool.http.HttpResponse} 为异步模式，执行完此方法后发送请求到服务器，但是并不立即读取响应内容。<br>
	 ...
	 */
	public com.relaxed.common.core.util.http.HttpResponse executeAsync() {
	 ...
	 * @return this
	 */
	public com.relaxed.common.core.util.http.HttpResponse execute(boolean isAsync) {
	 ...
	 * @since 5.7.8
	 */
	public void then(Consumer<com.relaxed.common.core.util.http.HttpResponse> consumer) {
		try (com.relaxed.common.core.util.http.HttpResponse response = execute(true)) {
	 ...
	 * 执行Reuqest请求
	 * @param isAsync 是否异步
	 * @param requestInterceptors 拦截器列表
	 * @return this
	 */
	public com.relaxed.common.core.util.http.HttpResponse doExecute(boolean isAsync,
			HttpInterceptor.Chain<HttpRequest> requestInterceptors,
			HttpInterceptor.Chain<com.relaxed.common.core.util.http.HttpResponse> responseInterceptors) {
		...
		if (null == httpResponse) {
			httpResponse = new com.relaxed.common.core.util.http.HttpResponse(this.httpConnection, this.charset,
					isAsync, isIgnoreResponseBody());
		}
		// 拦截响应
		if (null != responseInterceptors) {
			for (HttpInterceptor<com.relaxed.common.core.util.http.HttpResponse> interceptor : responseInterceptors) {
	 ...
	 * @param isAsync 是否异步
	 * @return {@link com.relaxed.common.core.util.http.HttpResponse}，无转发返回 {@code null}
	 */
	private com.relaxed.common.core.util.http.HttpResponse sendRedirectIfPossible(boolean isAsync) {
		// 手动实现重定向
