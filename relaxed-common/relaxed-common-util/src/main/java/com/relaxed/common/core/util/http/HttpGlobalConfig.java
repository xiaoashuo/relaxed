...
public class HttpGlobalConfig implements Serializable {
	...
	/**
	 * -1: 含义，永不超时。 如果：设置timeout = 3s(3000 ms), 那一次请求最大超时：就是：6s 官方含义：timeout of zero is
	 * interpreted as an infinite timeout. （0的超时被解释为无限超时。） 这里实际项目一定要进行修改，防止把系统拖死.
	...
	private static String boundary = "--------------------Relaxed_" + RandomUtil.randomString(16);
	...
	synchronized public static void allowPatch() {
		...
		if (null == methodsField) {
			throw new HttpException("None static field [methods] with Java version: [{}]",
					System.getProperty("java.version"));
		}

		// 去除final修饰
		ReflectUtil.setFieldValue(methodsField, "modifiers", methodsField.getModifiers() & ~Modifier.FINAL);
		final String[] methods = { "GET", "POST", "HEAD", "OPTIONS", "PUT", "DELETE", "TRACE", "PATCH" };
