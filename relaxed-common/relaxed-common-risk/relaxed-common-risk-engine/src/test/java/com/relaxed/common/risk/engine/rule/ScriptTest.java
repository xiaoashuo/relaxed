...
public class ScriptTest {
    ...
    public void testScript(){
        GroovyScriptHandler groovyScriptHandler = new GroovyScriptHandler();
         ...
         String ruleScript="class ActivationCheckScript {" +
                 " public boolean check (def context,def data,def lists) { " +
                 "def annexList=[ 1,2,7,9,12,20,21,24,25,35,36,37,38,42,45,54,69,74,76,77 ]; " +
                 "def annexParams=context.eventJson.annexPackagePath.split (\",\");" +
                 " for (param in annexParams){ " +
                 "if (! annexList.contains (param as Integer)){" +
                 " return true;" +
                 " } }" +
                 "return false;" +
                 "}}";
        ...
        maps.put("annexPackagePath","1,2,3");
        evaluateContext.setEventJson(maps);
        EvaluateReport evaluateReport=new EvaluateReport();
        GroovyResult groovyResult = groovyScriptHandler.invokeMethod(groovyScriptHandler.buildContext(ruleScript, "check", evaluateContext, evaluateReport, null));
