...
public abstract class AbstractInsertBatch extends AbstractMethod {
    ...
    protected String prepareFieldSql(TableInfo tableInfo) {
        ...
        String columnFields = tableInfo.getKeyInsertSqlColumn(true) + fieldList.stream().map(AbstractInsertBatch::getInsertSqlColumn)
    ...
    protected String prepareValuesSqlForMysqlBatch(TableInfo tableInfo) {
        ...
        String valueList = tableInfo.getKeyInsertSqlProperty(ENTITY_DOT, true) + fieldList.stream()
                ...
                .collect(Collectors.joining(NEWLINE));
        return  SqlScriptUtils.convertTrim(valueList,
